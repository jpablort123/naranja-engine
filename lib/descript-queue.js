...
import { createAgentJob, getJob, buildCutPrompt, interpretJob } from './descript';
...
export async function pollRunningJob(row) {
  ...
  try {
    ...
    const { outcome, compositionId, credits, errorMessage } = interpretJob(j);
    if (outcome === 'done') {
    ...
    if (outcome === 'error') {
      await completeJob(row.id, {
        status: 'error',
        error_message: errorMessage || 'Descript job failed',
        descript_response: j,
      });
      return 'error';
    }
