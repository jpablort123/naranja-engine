...
// ── El agente de Descript devuelve el id de la composición creada dentro del
//    texto de agent_response, como compositionId="<uuid>". Lo extraemos.
export function extractCompositionId(text) {
  if (!text || typeof text !== 'string') return null;
  const m = text.match(/compositionId="([0-9a-fA-F-]{36})"/);
  return m ? m[1] : null;
}

// ── Interpreta la respuesta REAL de un job de Descript (de GET /jobs/{id} o del
//    webhook, que comparten formato). Descript reporta:
//      job_state: 'queued' | 'running' | 'stopped' | 'cancelled'
//      result.status: 'success' | 'error'   (cuando job_state === 'stopped')
//    El id de la composición y los créditos viven dentro de `result`.
// Devuelve { outcome: 'running'|'done'|'error', compositionId, credits, errorMessage }.
export function interpretJob(payload) {
  // El webhook puede envolver el job en `data`.
  const j = (payload && (payload.job_state || payload.result || payload.job_id))
    ? payload
    : (payload?.data || payload || {});
  const jobState = (j?.job_state || j?.state || j?.status || '').toString().toLowerCase();
  const result = j?.result || {};
  const resultStatus = (result?.status || '').toString().toLowerCase();

  let outcome = 'running';
  if (jobState === 'cancelled') outcome = 'error';
  else if (jobState === 'stopped') outcome = resultStatus === 'error' ? 'error' : 'done';
  else if (['succeeded', 'done', 'completed', 'success'].includes(jobState)) outcome = 'done';
  else if (['failed', 'error'].includes(jobState)) outcome = 'error';

  const compositionId =
    result?.composition_id ||
    extractCompositionId(result?.agent_response) ||
    extractCompositionId(j?.agent_response) ||
    null;

  const credits =
    typeof result?.ai_credits_used === 'number' ? result.ai_credits_used :
    typeof j?.ai_credits_used === 'number' ? j.ai_credits_used :
    null;

  const errorMessage =
    result?.error?.message || result?.message || j?.error?.message || null;

  return { outcome, compositionId, credits, errorMessage, jobState };
}
